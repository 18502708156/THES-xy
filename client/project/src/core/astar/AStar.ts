class AStar {
     ...
     *
     */
	public initFromMap(map: MapGrid) {
		var nOldCount = 0;
	...
	public getPatch(fromX: number, fromY: number, targetX: number, targetY: number): AStarNode[] {
		...
		let acell = this.m_ASMapCells[targetY * this.m_nWidth + targetX]
		...
		let sing1 = targetX > fromX ? 1 : -1
		for (let index = 0, i = 0, len = Math.abs(targetX - fromX); i < len; i++) {
            let dx = sing1 + index
            let dy = Math.floor(dx * pos.y / pos.x);
		...
		if (line) {
			let sing2 = targetY > fromY ? 1 : -1;
            for (let index = 0, i = 0, len = Math.abs(targetY - fromY); i < len; i++) {
                let dy = sing2 + index;
                let dx = Math.floor(dy * pos.x / pos.y);
                if (!GameMap.checkWalkable(fromX + dx, fromY + dy)) {
                    line = false;
                    break;
                }
