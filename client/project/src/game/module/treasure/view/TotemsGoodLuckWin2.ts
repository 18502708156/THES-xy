 ...
 * 图腾寻宝
 */
class TotemsGoodLuckWin2 extends BaseView implements ICommonWindowTitle 
{

	public static NAME = "图腾寻宝"
	...
	childrenCreated() {
		...
		let config = GameGlobal.Config.TotemsLotteryConfig //TotemsLotteryBaseConfig
		...
		for (let i = 0; i < this.group.numChildren; i++) {
			...
			//Add Eff
			let effArr=GameGlobal.Config.TotemsLotteryBaseConfig.effitem;
...
private effDic={};
	...
	private _OnClick(e: egret.TouchEvent) {
		...
		switch (e.currentTarget) {
			...
			break
			case this.getway:
				let datas = GameGlobal.Config.TotemsLotteryBaseConfig.text
				let desc = []
	...
	private BuyHunt(type: number) {
		...
		let config = GameGlobal.Config.TotemsLotteryConfig[type][0]
